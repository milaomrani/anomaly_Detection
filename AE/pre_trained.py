# -*- coding: utf-8 -*-
"""
Created on Wed Mar 23 22:41:01 2022
...
import numpy as np
#%%
img_dir = 'E:/DATASET/carpet/carpet/'
SIZE = 224 
dataset = []
label = []

#%%
anomaly_images = os.listdir(img_dir + 'train/bad/')
for i, image_name in enumerate(anomaly_images):    #Remember enumerate method adds a counter and returns the enumerate object
    
    if (image_name.split('.')[1] == 'png'):
        image = cv2.imread(img_dir + 'train/bad/' + image_name)
...
good_images = os.listdir(img_dir + 'train/good/')
for i, image_name in enumerate(good_images):
    if (image_name.split('.')[1] == 'png'):
        image = cv2.imread(img_dir + 'train/good/' + image_name)
        image = Image.fromarray(image, 'RGB')
        image = image.resize((SIZE, SIZE))
        dataset.append(np.array(image))
        label.append(0)
...
X_train, X_test, y_train, y_test = train_test_split(dataset, label, test_size = 0.15, random_state = 0)
...
#n=29  #Select the index of image to be loaded for testing
img = X_test[n]
plt.imshow(img)
